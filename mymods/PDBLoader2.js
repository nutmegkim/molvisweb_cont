...
const bondLengths = { 
    C: {
        C: {single: 1.54, double: 1.34, triple: 1.21}, 
        H: {single: 1.10, double: null, triple: null}, 
        N: {single: 1.46, double: 1.27, triple: 1.16},
        ...
        H: {
            H: {single: 0.74, double: null, triple: null},
            N: {single: 1.00, double: null, triple: null},
...
//Betty made changes to the isBond function
		function isBond(atom1, atom2, distance) {
            ...
            function threshold(num1, num2, range){
             ...
             function getBondLengths(atom1, atom2){
             ...
             function bondOrder(single, double, triple, sdThresh, dtTresh){
                ...
                else if((double != null) && (distance > (double - DT_thresh)) && (distance < (double + SD_thresh))) {
                     double_bond = true; 
                }

                else if((triple != null) && (distance > (triple-range)) && (distance < (triple + DT_thresh))) {
                    triple_bond = true; 
                }

                else if ((single != null) || (double != null) || (triple != null)){
                    partial_bond = true;
                }
                else{
                     console.log("bond can't be formed")
                 }


              }

              function bondH(single){

                 if(Math.abs(distance - single) < range){
                        bond = true; 
                 }
                 else{
                        console.log("Bond length is out of typical range");
                    }
              }

              if (atom1 == "H" || atom2=="H"){
                const single = getBondLengths(atom1, atom2)
                bondH(single)
              }

              else{

                const {single, double, triple, SD_thresh, DT_thresh} = getBondLengths(atom1, atom2)

                bondOrder(single, double, triple, SD_thresh, DT_thresh)
 ...
 function buildGeometry() {
            ...
            for ( let i = 0, l = atoms.length; i < l; i ++ ) {
                ...
                for ( let j = i+1; j < atoms.length; j += 1 ) {
                    ...
                    else if (is_partialbond) order = 1.5 //??
                    ...
                    if(order > 0){
                        ...
                        if ( _bhash_manual[ h ] === undefined ) {
							...
							if (is_singlebond){
                                ...
                                //choose non-parallel arbitrary vector to find perpendicular vector to bond vector
                                let arbitrary = new Vector3(1, 0, 0);
                                                            
                                //if arbitrary vector is parallel to bond vector, choose another vector
                                if (Math.abs(bondDir.dot(arbitrary)) > 0.9){
